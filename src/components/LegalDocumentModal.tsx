...
const TermsBody = () => (
  <div className="space-y-4">
    <p><strong>Truprice.ng provides price intelligence, not a guarantee of a seller's final price.</strong> Prices can change by vendor, location, quantity, quality, and time of day.</p>
    <section><h3 className="font-bold mb-1">Using Truprice.ng</h3><p>Use the service lawfully and do not submit false, manipulated, duplicate, or misleading information. Public users may report suspected inaccuracies; reports do not directly overwrite published prices.</p></section>
    <section><h3 className="font-bold mb-1">Verified price observations</h3><p>Published prices are calculated from observations that pass Truprice.ng's collection and verification workflow. A confidence score describes the available evidence; it is not a promise that every seller will quote the same amount.</p></section>
    <section><h3 className="font-bold mb-1">Field agents and administrators</h3><p>Field-agent access is assigned by Truprice.ng. Agents must collect genuine market evidence and location data. Verifier/admin access is restricted and may be revoked when necessary to protect data integrity.</p></section>
    <section><h3 className="font-bold mb-1">Accounts</h3><p>You are responsible for keeping your login credentials secure. Truprice.ng may deactivate accounts used to abuse the service, compromise data quality, or attempt unauthorized access.</p></section>
    <section><h3 className="font-bold mb-1">Service changes</h3><p>Truprice.ng may update product coverage, markets, methodology, or these terms as the service develops. When a material new version requires renewed acceptance, the app will ask you again.</p></section>
  </div>
);

const PrivacyBody = () => (
  <div className="space-y-4">
    <p>Truprice.ng collects only the information needed to operate accounts, personalize the product, protect the price-verification network, and improve data quality.</p>
    ...
    <section><h3 className="font-bold mb-1">Consumer activity</h3><p>When signed in, Truprice.ng may store saved products and inaccurate-price reports so those actions follow your account.</p></section>
    ...
    <section><h3 className="font-bold mb-1">Access and retention</h3><p>Truprice.ng uses role-based access controls so consumers, field agents, verifiers, and the super admin receive different levels of access. Operational records and audit history may be retained when needed to preserve the integrity of the price-verification system.</p></section>
...
export const LegalDocumentModal: React.FC<{ document: LegalDocument | null; onClose: () => void }> = ({ document, onClose }) => {
  ...
  const title = document === 'terms' ? 'Truprice.ng Terms of Use' : 'Truprice.ng Privacy Notice';
  return (
